...
# 프로젝트 개요
  * 최근 PC방은 컴퓨터 이용요금보다 먹거리 판매 수익이 점점 증대되는 추세로, 많게는 50~60%의 매출이
식음료 판매에서 발생하고 있다고 합니다. 이러한 먹거리 수익을 한층 더 발전시키고자 자체 웹 프로그램
"<PC_EAT>" 을 구현하게 되었습니다. 해당 앱에서 고객은 PC방에서만 맛볼 수 있었던 특정 메뉴를 집에서도
배달시켜 먹을 수 있는 서비스를 제공받을 수 있습니다. 시중의 일반 배달 앱과 달리, 포인트 적립 및 할인 혜택
등 단골 관리의 편의성을 추가하여 차별점을 두었습니다.

# 기획 배경(사용자와 pc방 점주)
 * 필요 이상의 지출: PC방 내에서 판매되는 음식은 외부 음식점보다 가격이 비싼 경우가 많습니다.(사용자의 지출)   
 * pc방 사용자의 인원 즘감: 코로나시대 이후 pc방 이용인원과 사용시간을 증감
 ...
 * 포인트 자동 적립
 ...
 # 순서도
  * <img src="./PcEat num.png" alt="PCEat DB" width="500" height="400"/>
  ...
  * 역할 분담
   ...
   * 코드 버전 관리:
   ...
   1. 홈화면
      ...
      * 로그인한 사용자라면 별도의 절차 없이 모든 기능을 즉시 이용 가능
      * 상단에는 사용자 이름, 포인트 잔액, 알림 아이콘 등을 표시하여 편의성 강화
   2. 주문 페이지
      1. 사용자는 원하는 메뉴를 선택하여 장바구니에 추가
      2. 장바구니 확인 후, 간편하게 결제 진행
      3. 결제 완료 시, “포인트 적립 완료” 안내 팝업이 즉시 표시되어 적립 내역을 사용자에게 명확히 전달
      4. 직관적인 흐름을 통해 사용자의 편의성과 만족도 향상 
   3. 마이페이지/이벤트
      * 마이페이지
        * 사용자는 자신의 포인트 적립 및 사용 내역을 한눈에 확인 가능
        * 보유 쿠폰과 사용 가능 여부, 만료일 등 상세 정보 제공
        * 주문 내역, 최근 이용 기록 등 개인화 정보 확인 가능
      * 이벤트 페이지
        * 현재 진행 중인 이벤트 목록과 참여 조건을 직관적으로 확인
        * 포인트를 사용해 즉시 응모 가능한 경품형 이벤트 또는 미션형 프로모션 참여
        * 당첨 결과나 참여 내역을 실시간으로 확인할 수 있는 기능 제공
    
      # 기대효과
        * 웹 기반 포인트 적립 및 사용 시스템을 통해 고객 편의성이 향상되고, 재방문을 유도할 수 있음
        * 기존 PC방과 차별화되는 부가 서비스 제공으로 브랜드 경쟁력 강화
        * 적립 포인트와 쿠폰을 활용한 이벤트 마케팅 전략이 가능해져 고객 참여도와 매출 상승 효과 기대
        * 사용자 데이터 기반으로 고객 맞춤형 혜택 및 프로모션 운영 가능
